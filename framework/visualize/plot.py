...
class MatplotlibPlot(CustomPlot):
    ...
    def to_wandb(self):
        ...
        if self.as_image:
            plotf = FigureCanvas(plot)
            plotf.draw()
            plotf = PIL.Image.frombuffer('RGBA', plotf.get_width_height(), plotf.buffer_rgba(), 'raw', 'RGBA', 0, 1)
            plotf = wandb.Image(plotf)
            plt.close(plot)
            plot = plotf
        else:
            newplot = wandb.Plotly.make_plot_media(plot)
            plt.close(plot)
            plot = newplot
...
class Heatmap(MatplotlibPlot):
    def __init__(self, map: Union[torch.Tensor, np.ndarray], xlabel: Optional[str] = None, ylabel: Optional[str] = None,
                 ...
                 cmap="auto", colorbar: bool = True, vrange=(None, None), figsize=None, ticksize: int = 8,
                 flip_y: bool = False):
        ...
        if flip_y:
            ...
            if self.y_marks is None:
                self.y_marks = [str(i) for i in range(self.map.shape[0])]
            self.y_marks = list(reversed(self.y_marks))
...
class AnimatedHeatmap(CustomPlot):
    def __init__(self, map: Union[torch.Tensor, np.ndarray], xlabel: str, ylabel: str,
                 ...
                 colorbar_labels = None, ignore_wrong_marks: bool = False, fontsize: float = 6,
                 shape_factor: float = 0.1, shape: Optional[Tuple[int,int]] = None, vrange=(None, None)):
    ...
    def to_video(self):
        ...
        colorbar_pad = 0.1
        if self.colorbar:
            colorbar_size_offset = colorbar_size + colorbar_pad
        else:
            colorbar_size_offset = 0

        minsize = 1.5

        shape = self.shape
        if shape is None:
            shape = (max(self.shape_factor*data[0].shape[-1], minsize) + colorbar_size_offset, max(self.shape_factor*data[0].shape[0], minsize))

        figure, ax = plt.subplots(figsize=shape)
        canvas = FigureCanvas(figure)
        figure.set_tight_layout(True)

        vmin = self.range[0] or data.min()
        vmax = self.range[1] or data.max()

        im = plt.imshow(data[0], interpolation='nearest', cmap=self.cmap, aspect='auto', animated=True,
                        vmin = vmin, vmax=vmax)

        if x_marks is not None:
            plt.xticks(np.arange(self.map.shape[2])[::self.subsample_ticks], x_marks, rotation=45, fontsize=self.font_size,
                    ha="right", rotation_mode="anchor")

        if y_marks is not None:
            plt.yticks(np.arange(self.map.shape[1])[::self.subsample_ticks], y_marks, fontsize=self.font_size)
        ...
        if self.textval:
            ...
            for i, j in itertools.product(range(self.map.shape[0]), range(self.map.shape[1])):
                color = "white" if self.map[i, j] > threshold else "black"
                plt.text(j, i, rmap[i, j], ha="center", va="center", color=color, fontsize=self.font_size)
