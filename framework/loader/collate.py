...
class VarLengthCollate:
    def __init__(self, ignore_symbol=0, batch_dim: int = 1, pad_to: Optional[List[int]] = None):
        self.ignore_symbol = ignore_symbol
        self.batch_dim = batch_dim
        self.pad_to = pad_to

    def _measure_array_max_dim(self, batch: List[torch.Tensor]):
        s=list(batch[0].size())
        if self.pad_to is not None and len(s) == len(self.pad_to):
            for i, m in enumerate(self.pad_to):
                s[i] = max(m, s[i])
