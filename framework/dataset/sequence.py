...
class SampleTracker:
    ...
    def get(self, limit: Optional[int] = None) -> visualize.plot.TextTable:
        return visualize.plot.TextTable([n for n, e in zip(self.header, self.enabled) if e],
                                        self.pick_seqeunces(limit))


class SequenceTestState:
    def __init__(self, batch_dim: int = 1, ignore_index: int = -100):
        self.cntr = AccuracyCounter()
        self.batch_dim = batch_dim
        self.time_dim = 1 - self.batch_dim
        self.ignore_index = ignore_index

    def get_out(self, data: Dict[str, torch.Tensor], mask: bool = True) -> torch.Tensor:
        out = data["out"].long()
        if mask and ("mask" in data):
            out = out.masked_fill(~data["mask"].bool(), self.ignore_index)
        return out
    ...
    def compare_direct(self, net_out: Tuple[torch.Tensor, Optional[torch.Tensor]], ref: torch.Tensor,
        ...
        if len is not None:
            ...
            ok_mask = ((out == ref) | unused | (ref == self.ignore_index)).all(self.time_dim) & (len == ref_len)
        else:
            # Allow fixed lenght output
            assert out.shape==ref.shape
            ok_mask = ((out == ref) | (ref == self.ignore_index)).all(self.time_dim)

        return ok_mask

    def compare_output(self, net_out: Tuple[torch.Tensor, Optional[torch.Tensor]], data: Dict[str, torch.Tensor]):
        return self.compare_direct(net_out, self.get_out(data), data["out_len"])
...
class SampleTrackerTestState(SequenceTestState):
    def __init__(self, batch_dim: int, max_bad_samples: int = 100, max_good_samples: int = 0,
                 type_names: Optional[List[str]] = None, ignore_index: int = -100):
        super().__init__(batch_dim=batch_dim, ignore_index=ignore_index)
...
class TextClassifierTestState(SampleTrackerTestState):
    ...
    def ref_to_text(self, data: Dict[str, torch.Tensor], i: int) -> Tuple[str, str]:
        out = self.get_out(data, False)
        t_ref = self.out_to_text(out[i].item())
    ...
    def step(self, net_out: torch.Tensor, data: Dict[str, torch.Tensor]):
        out = self.convert_to_index(net_out)
        target = self.get_out(data)
        ok_mask = (out == target) | (target == self.ignore_index)
...
class TextSequenceTestState(SampleTrackerTestState):
    ...
    def __init__(self, input_to_text: Callable[[torch.Tensor], torch.Tensor],
                 ...
                 add_eos: bool = True, ignore_index: int = -100):
        super().__init__(batch_dim, max_bad_samples, max_good_samples, type_names, ignore_index=ignore_index)
    ...
    def loss(self, net_out: torch.Tensor, data: Dict[str, torch.Tensor]) -> torch.Tensor:
        out = self.get_out(data)
        ref = add_eos(out, data["out_len"], net_out.shape[-1] - 1) if self.add_eos else out

        # If net out is longer than the ref, just ignore the extra chars.
        net_out = net_out[:ref.shape[0]]

        mask = torch.arange(net_out.shape[1 - self.batch_dim], device=net_out.device).unsqueeze(1) < \
               (data["out_len"].unsqueeze(0) + int(self.add_eos))

        mask = mask & (ref != self.ignore_index)
    ...
    def ref_to_text(self, data: Dict[str, torch.Tensor], i: int) -> Tuple[str, str]:
        out = self.get_out(data, mask=False)
        t_ref = self.out_to_text(out.select(self.batch_dim, i)[: int(data["out_len"][i].item())].
                cpu().numpy().tolist())
        t_in = self.in_to_text(data["in"].select(self.batch_dim, i)[: int(data["in_len"][i].item())].cpu().numpy().
                               tolist())
        return t_in, t_ref
    ...
    def update_len_stats(self, stats: Dict[int, Dict[str, float]], ok_mask: torch.Tensor, len: torch.Tensor):
        for l in len.unique():
            l = l.item()
            l_mask = len == l
            ...
            stats[l]["ok"] += ok_mask[l_mask].sum().item()
            stats[l]["cnt"] += l_mask.sum()
    ...
    def step(self, net_out: Tuple[torch.Tensor, Optional[torch.Tensor]], data: Dict[str, torch.Tensor]):
        ...
        if not self.is_index_tensor(scores):
            ...
            self.update_len_stats(self.stat_per_out_len, ok_mask, data["out_len"])
        else:
            oracle_ok = None
        ...
        prefix_ok_mask = self.compare_direct((net_out[0], prefix_len), target, prefix_len)
    ...
    def plot(self) -> Dict[str, Any]:
        ...
        if self.losses:
            res["loss_histogram"] = visualize.plot.Histogram(torch.cat(self.losses, 0))

        for l, s in self.stat_per_out_len.items():
            res[f"accuracy_per_length/{l}"] = s["ok"] / s["cnt"]
